            ...
            self.transformations = Compose(
                [
                    ...
                    CoarseDropout(
                        ...
                        min_holes=1,
                        max_holes=1,
                        fill_value=means,  # Need to verify. Since we are normalizing the values before this augmentation, we can just use means directly without *255
                    ),
                    ToTensorV2(),
                ]
            )
...
def train(model, device, train_loader, optimizer, criterion, train_losses, train_acc, lr_history, scheduler):
    """
...
def test(model, device, test_loader, criterion, test_losses, test_acc):
    ...
    with torch.no_grad():  # Disable gradient calculation
        for batch_idx, (data, target) in enumerate(test_loader):
            ...
            test_loss += criterion(output, target).item()  # Sum up batch loss. If we use cross-entropy loss, TypeError: CrossEntropyLoss.forward() got an unexpected keyword argument 'reduction
...
def training(model, device, num_epochs, train_loader, test_loader, optimizer, criterion, scheduler):
    ...
    for epoch in range(1, num_epochs+1):
        print(f'Epoch {epoch}')
        train(model, device, train_loader, optimizer, criterion, train_losses, train_acc, lr_history, scheduler)
    ...
    return train_losses, test_losses, train_acc, test_acc, lr_history
...
def get_lr(
    ...
    max_lr = lr_finder.history["lr"][np.argmin(lr_finder.history["loss"], axis=0)]  # What for?
